...
if __name__ == "__main__":
    ...
    for metric in ["kills", "deaths", "assists"]:
        ...
        working_df[f"team1_alive_{metric[0]}"] = (
            combined_df[[f"player{i}_{metric}" for i in range(1, 6)]]
            .multiply(np.array(combined_df[[f"player{player_number}_alive" for player_number in range(1, 6)]]))
            .sum(axis=1)
        )
        working_df[f"team2_alive_{metric[0]}"] = (
            combined_df[[f"player{i}_{metric}" for i in range(6, 11)]]
            .multiply(np.array(combined_df[[f"player{player_number}_alive" for player_number in range(6, 11)]]))
            .sum(axis=1)
        )
    ...
    for metric in ["k", "d", "a"]:
        ...
        # team1
        features_df[f"team1_perc_{metric}_of_team"] = (
            working_df[f"team1_alive_{metric}"] / working_df[f"team1_{metric}"]
        )
        features_df[f"team1_perc_{metric}_of_total"] = (
            working_df[f"team1_alive_{metric}"] / working_df[f"total_{metric}"]
        )

        # team2
        features_df[f"team2_perc_{metric}_of_team"] = (
            working_df[f"team2_alive_{metric}"] / working_df[f"team2_{metric}"]
        )
        features_df[f"team2_perc_{metric}_of_total"] = (
            working_df[f"team2_alive_{metric}"] / working_df[f"total_{metric}"]
        )
