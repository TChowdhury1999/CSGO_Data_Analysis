...
def add_pistol_round(main_df):
    ...
    main_df.loc[
        ((main_df["last_round"] == 8) | (main_df["last_round"] == 9)) & (main_df["score_sum"] == 9), "pistol_round"
    ] = 1
    ...
    main_df.loc[
        ((main_df["last_round"] == 8) | (main_df["last_round"] == 9)) & (main_df["round"] > 8), "team1_t_side"
    ] = False

    # equivalent for team2 is just boolean opposite

    main_df["team2_t_side"] = ~main_df["team1_t_side"]
...
# generate round df
round_df = gen_main_df()


# convert this to a per kill time instance df
# score | players alive | time |
###############################################################################
# ANALYSIS
###############################################################################*
